...
struct binary_nameless_traits {
    ...
    static void do_struct(Ar& ar, T& o, prefix pre, Context& ctx) {
        access::call<T>::pick_serialize(ar, o, pre.version);
    }

    template<class Ar, class T, class Context>
    static void do_struct(Ar& ar, nv_t<T>& o, prefix pre, Context& ctx) {
    ...
    template<class Ar, class T, class Context>
    static void do_struct_array(Ar& ar, T* o, size_t size, prefix pre, Context& ctx) {
        for (size_t i=0; i<size; ++i)
            access::call<T>::pick_serialize(ar, o[i], pre.version);
...
struct yaml_write_traits {
    ...
    template<class Ar, class T, class Context>
    static void do_struct(Ar& ar, T& o, prefix pre, named<Context>& n) {
