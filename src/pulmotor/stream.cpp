...
sink_vector::sink_vector(sink_vector const& s) : m_data(s.m_data) {
}
sink_vector::~sink_vector() {
}

void sink_vector::write(void const* data, size_t size, std::error_code& ec) {
    ec.clear();
    m_data.insert(m_data.end(), (char const*)data, (char const*)data + size);
}
...
static int get_opfl(source_mmap::flags flags) { return flags==source_mmap::ro ? O_RDONLY : flags==source_mmap::wr ? O_WRONLY : O_RDWR; }
static int get_protfl(source_mmap::flags flags) { return flags==source_mmap::ro ? PROT_READ : flags==source_mmap::wr ? PROT_WRITE : (PROT_READ|PROT_WRITE); }
...
void source_mmap::map(path_char const* path, flags flags, std::error_code& ec, fs_t off, size_t mmap_block_size)
{
    m_flags = flags;
...
void source_mmap::make_available(std::error_code& ec)
{
    ...
    if (willMap != 0) {
        int protfl = get_protfl(m_flags);
        if ((m_mmap = m_data = (char*)mmap(NULL, willMap, protfl, MAP_SHARED, m_fd, m_bloff)) == (void*)-1) {
            ec=mk_ec(errno);
            reset();
            zero_block();
            m_blsize=0;
